...
def connect_to_libreoffice_server():
    """실행 중인 LibreOffice 서버에 연결"""
    try:
        # 로컬 컨텍스트 생성
        ...
        print(f"LibreOffice 서버 연결 실패: {e}")
        print("다음 명령으로 서버를 먼저 시작하세요:")
        print("./scripts/uno/start_libreoffice_server.sh")
        raise


def list_sheets_server(xlsx_path):
    """서버 연결 방식으로 Excel 파일의 시트 목록 가져오기"""
    if not UNO_AVAILABLE:
        raise ImportError("LibreOffice UNO가 설치되지 않았습니다.")
    
    try:
        # LibreOffice 서버에 연결
        ctx = connect_to_libreoffice_server()
...
if __name__ == "__main__":
    try:
        ...
        # 시트 목록 출력
        sheets = list_sheets_server(excel_file)
